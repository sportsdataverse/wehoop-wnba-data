...
wnba_team_box_games <- function(y) {
  ...
  if (length(season_team_box_list) > 0) {
    cli::cli_progress_step(
      msg = "Compiling {y} ESPN WNBA Team Boxscores ({length(season_team_box_list)} games)",
    ...
    espn_df <- furrr::future_map_dfr(
      ...
      function(x) {
        tryCatch(
          ...
          error = function(e) {
            message(glue::glue(
              "{Sys.time()}: Team box score data issue for {x}!"
            ))
          }
        )
      },
    ...
    cli::cli_progress_step(
      msg = "Updating {y} ESPN WNBA Team Boxscores GitHub Release",
  ...
  if (nrow(espn_df) > 0) {
    espn_df <- espn_df %>%
      ...
      wehoop:::make_wehoop_data(
        "ESPN WNBA Team Boxscores from wehoop data repository",
        Sys.time()
      )

    ifelse(
      !dir.exists(file.path("wnba/team_box")),
      dir.create(file.path("wnba/team_box")),
      FALSE
    )

    ifelse(
      !dir.exists(file.path("wnba/team_box/csv")),
      dir.create(file.path("wnba/team_box/csv")),
      FALSE
    )
    data.table::fwrite(
      espn_df,
      file = paste0("wnba/team_box/csv/team_box_", y, ".csv.gz")
    )

    ifelse(
      !dir.exists(file.path("wnba/team_box/rds")),
      dir.create(file.path("wnba/team_box/rds")),
      FALSE
    )
    saveRDS(espn_df, glue::glue("wnba/team_box/rds/team_box_{y}.rds"))

    ifelse(
      !dir.exists(file.path("wnba/team_box/parquet")),
      dir.create(file.path("wnba/team_box/parquet")),
      FALSE
    )
    arrow::write_parquet(
      espn_df,
      glue::glue("wnba/team_box/parquet/team_box_{y}.parquet")
    )
  ...
  if (nrow(espn_df) > 0) {
    ...
    cli::cli_alert_info(
      "{length(season_team_box_list)} ESPN WNBA Team Boxscores to be compiled for {y}, skipping Team Boxscores compilation"
    )
